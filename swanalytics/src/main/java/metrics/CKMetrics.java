...
public class CKMetrics {
	...
	public static boolean isInterfaceInheritanceImplemented(ClassBean cb) {

		if(cb.getTextContent().contains(" implements ")) {
			return true;
		}
	...
	public static boolean isImplementationInheritanceImplemented(ClassBean cb) {

		if(cb.getTextContent().contains(" extends "))
			return true;

		return false;
	}

	public static int getDelegation(MethodBean mb) {
		int delegation = 0;
		Collection<MethodBean> calls = mb.getMethodCalls();

		for(MethodBean call : calls) {
			if (call.getUsedInstanceVariables().size() > 0)
				delegation++;
		}
	...
	public static int getDelegation(ClassBean cb) {
		int delegation = 0;

		for(MethodBean mb: cb.getMethods()) {
			for(MethodBean call : mb.getMethodCalls()) {
				delegation++;
			}
		}
